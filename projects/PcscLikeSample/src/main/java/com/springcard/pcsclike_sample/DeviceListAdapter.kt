...
class DeviceListAdapter(private val context: Context,
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val rssi = device.info.toIntOrNull()
        if (rssi != null) {
            viewHolder.signalImageView.visibility = View.VISIBLE
            when {
                rssi < -100 -> viewHolder.signalImageView.setImageResource(R.drawable.ic_signal_cellular_0_bar)
                rssi < -75 -> viewHolder.signalImageView.setImageResource(R.drawable.ic_signal_cellular_1_bar)
                rssi < -50 -> viewHolder.signalImageView.setImageResource(R.drawable.ic_signal_cellular_2_bar)
                rssi < -25 -> viewHolder.signalImageView.setImageResource(R.drawable.ic_signal_cellular_3_bar)
                else -> viewHolder.signalImageView.setImageResource(R.drawable.ic_signal_cellular_4_bar)
            }
        } else {
            viewHolder.signalImageView.visibility = View.GONE
        }
