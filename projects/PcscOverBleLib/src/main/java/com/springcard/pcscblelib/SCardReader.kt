...
class SCardReader internal  constructor(val parent: SCardReaderList) {
    ...
    private fun getStatus() {
        parent.process(ActionEvent.ActionWriting(GattAttributesSpringCore.UUID_CCID_PC_TO_RDR_CHAR, parent.ccidHandler.scardStatus(index)))
    }

    /* Deprecated */
    private fun cardDisconnect() {
        parent.process(ActionEvent.ActionWriting(GattAttributesSpringCore.UUID_CCID_PC_TO_RDR_CHAR, parent.ccidHandler.scardDisconnect(index)))
    }
    ...
    fun cardConnect() {
        if(cardPowered && cardPresent) {
            parent.mHandler.post{ parent.callbacks.onCardConnected(channel) }
        }
        else {
            parent.process(ActionEvent.ActionWriting(GattAttributesSpringCore.UUID_CCID_PC_TO_RDR_CHAR, parent.ccidHandler.scardConnect(index)))
        }
    }
